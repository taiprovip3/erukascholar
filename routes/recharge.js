...
router.post('/gb/ibanking/create-payment', async (req, res) => {
    try {
        ...
        if(gbReponseData.code == 1) {
            const newTransaction = new Transaction({ userId, transId, amount, bankCode, message, status: 'Đã nhận mã code: "01". Đang chờ via third party trả kết quả về..' });
...
router.post('/tst/callback', async (req, res) => {
    try {
        ...
        if(!transaction) {// nếu không tìm thấy transaction
            ...
            if(!updateTransaction) {
                logger.info(`can't update failed transactions: ${content} status to failed because update transaction stament is fail!`);
            }
        ...
        if(dataCallback.status != 'thanhcong') {
            ...
            const newTransactionDataField = {
                status: `[${new Date().toLocaleString()}] Máy chủ đã nhận phản hồi third party và tìm thấy mã giao dịch ${content}. Nhưng giao dịch thất bại do: ${dataCallback.status}.`
            }
