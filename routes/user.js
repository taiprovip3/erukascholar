...
router.put('/profile/update', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        await clientQuery.query('UPDATE profiles SET fullname = $1, sdt = $2, country = $3, address = $4 WHERE users_id = $5', [fullname, sdt, country, address, userId]);
...
router.post('/profile/verify', authenticateToken, async (req, res) => {
    ...
    try {
        ...
        const isEmailUsedQuery = await clientQuery.query('SELECT * FROM users WHERE email = $1 AND is_verified = TRUE', [email]);
        if(isEmailUsedQuery.rowCount > 0) {// Đã có người dùng email này
            const sweetReponse = {
